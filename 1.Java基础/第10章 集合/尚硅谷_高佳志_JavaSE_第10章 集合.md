...
接下来我们通过案例学习如何使用Ite'rator迭代集合中元素：
...
2. ### java.lang.Iterable接口
   ...
   自定义某容器类型，实现java.lang.Iterable接口，发现就可以使用foreach进行迭代。即自定义类通过实现Iterable接口使用增强gfor循环
...
> 所以也不要在foreach遍历的过程使用Collection的·emove()方法。否则，要么报异常java.util.ConcurrentModificationException，要么行为不确定。
...
* void add(int index, E ele)：原来位置的元素向后顺延，不会发生替换
...
**Vector实现类**底层也使用数组，但是线程安全，效率低，不推荐使用。

```
Vector和ArrayList的相同点和不同点
相同点：
1.共同的父亲 AbstractList
2. Object[] elementData;底层都是Object[] 存储数据
不同点：
1.创建Vector对象是，已经开辟长度为10 的数组
  创建ArrayList对象时，底层数组的长度为0
  
  ArrayList在首次添加数据时扩容为10，之后扩容时时扩容到原来的1.5倍：int newCapacity + (oldCapacity >> 1);
  Vector扩容规则
  int newCapacity = oldCapacity + ((capacityIncrement > 0) ? capacityIncrement : oldCapacity);
  如果 capacityIncrement <= 0 Vector扩容到原来的2倍
  如果 capacityIncrement >0 Vector扩容到oldCapacity + capacityIncrement;
...
### 10.6.2 TreeSet：有序，唯一（指的是自然顺序---从小到大）
